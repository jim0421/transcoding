...
static void fshGetPeriod(char* filename);
...
int main(int argc, char **argv)
{
    ...
    strcpy(ess[1].outputName, output); 
    strcat(ess[1].outputName, "_2.pcm");
    
    fshGetPeriod(ess[0].inputName);
    /* create threads */
    pthread_t* threads = (pthread_t*) (malloc(THREAD_CNT * sizeof(pthread_t)));
    for (int i = 0; i < THREAD_CNT; i++){
        pthread_create(&threads[i], NULL, (void * (*)(void *))fshDecode, (void *)&ess[i]);
    }
    for (int i = 0; i < THREAD_CNT; i++){
...
static void fshGetPeriod(char* filename)
{
    ...
    int fz = ftell(fp);
    printf("pivot fp is @ position %ld\n",fz);
}
...
static void fshDecode(void* args)
{
    ...
    while (avpkt.size > 0) {
        ...
        // every avpkt has different size
        int i, ch;
        int got_frame = 0;
        ...
        if (avpkt.size < AUDIO_REFILL_THRESH) {//4096
            ...
            avpkt.data = inbuf;
            /*
             *if (fshBufferSize < (AUDIO_INBUF_SIZE-avpkt.size)){
